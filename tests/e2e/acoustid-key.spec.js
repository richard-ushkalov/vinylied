...
test('ключ из настроек: работает — так и написано', async ({ page }) => {
    ...
    await withAcoustIdKey(page, '');
    await page.goto('/');
    await openSettings(page);
    const status = page.locator('#settings-sheet .field:has(#setting-acoustidKey) [data-bind="status"]');
...
test('AcoustID не принял ключ — видно, почему, и поиск идёт через iTunes', async ({ page }) => {
    ...
    await withAcoustIdKey(page, 'UserKey123');
    await page.goto('/');
